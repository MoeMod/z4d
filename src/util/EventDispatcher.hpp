...
public:
	...
	// 发布订阅，返回第一个满足指定条件的结果
	template<class Pred, class ResultType = R>
	ResultType dispatch_find_if(typename ParseArg<Args>::type...args, Pred p, ResultType notFound = {})
	{
		const std::vector<std::shared_ptr<ICallable>> sv = makeCallableSet();
		auto iter = std::find_if(sv.begin(), sv.end(), [&args..., &p, &notFound](const std::shared_ptr<ICallable>& sp) mutable { return p(notFound = (*sp)(args...)); });
		applyCallableSet({ sv.begin(), sv.end() });
		return notFound;
	}
